module.exports = app => {
    ...
    // const Article = require('../../models/Article')  可用以下方法代替
    ...
    router.get('/news/init', async (req, res) => {
        ...
        const newsList = newsTitles.map(title => {
            const randomCats = cats.slice(0).sort((a, b) => Math.random() - 0.5) // 用slice(0)返回一个新数组
            return {
    ...
    router.get('/heroes/:id', async (req, res) => {
        const data = await Hero.findById(req.params.id).populate('categories item1 item2 partners.hero restrained.hero').lean()
        res.send(data)
    })
